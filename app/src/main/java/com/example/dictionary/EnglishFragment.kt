...
import com.example.dictionary.databinding.FragmentEnglishBinding
import com.google.firebase.firestore.FirebaseFirestore

class EnglishFragment : Fragment() {
    lateinit var binding: FragmentEnglishBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentEnglishBinding.inflate(LayoutInflater.from(context))
        firestore = FirebaseFirestore.getInstance()
        MySharedPreference.init(context)

        if (MySharedPreference.engWords.isEmpty()) {
            ...
            firestore.collection("word").get().addOnCompleteListener {
                if (it.isSuccessful)
                    for (document in it.result) {
                        ...
                        MySharedPreference.engWords = words
                    }
            }
        }else{
            binding.rv.adapter =
                RvAdapter(requireContext(), MySharedPreference.engWords, object : RvAdapter.ItemListener {
                    override fun onClick(position: Int) {
                        findNavController().navigate(
                            R.id.infoFragment,
                            bundleOf(
                                "desc" to MySharedPreference.engWords[position].descWord,
                                "title" to MySharedPreference.engWords[position].titleWord
                            )
                        )
                    }
